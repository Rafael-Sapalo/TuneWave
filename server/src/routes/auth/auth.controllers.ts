...
export class AuthController {
    ...
    async (ctx) => {
        ...
        const user: unknown = await db.insert(UserTable).values(
            {
    ...
    readonly loginFactory = this.authFactory.createHandlers(zValidator('json', UserLogSchema), async (ctx) => {
        ...
        if (!user) return ctx.json({ message: authErrorMessage[2] as string }, statusCodes.C400.NOT_FOUND);
        const compare = await passwordSec.comparing(body.password.toString(), user[0].password);
        if (!compare) return ctx.json({ message: authErrorMessage[3] }, statusCodes.C400.BAD_REQUEST);
